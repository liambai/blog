...
A good model (baby) at reconstruction gets it exactly right with high probability. Given some input $\mathbf{x}$, let's pick some random $\mathbf{z_{rand}}$ and look at $p(\mathbf{x}|\mathbf{z_{rand}})$: the probability of reconstructing the input perfectly. We want this number to be big.
...
Here's another way to look at the reconstruction problem. Every time the baby hears and repeats a word, he makes some _update_ to his internal view of the world: a distribution over the latent space, $p(z)$. Learning is nothing but _a series of these updates_.
...
$p(z)$ is the **prior distribution** (before the update) and $p(z | \mathbf{x})$ is the **posterior distribution** (after the update). With each observation, the baby computes the posterior and uses it as the prior for the next observation. This is often called **Bayesian inference** because we use **Bayes rule** to compute the posterior:

$$
p(z | \mathbf{x}) = \frac{p(\mathbf{x} | z) p(z)}{p(\mathbf{x})}
$$

This formula seems obvious from the manipulation of math-symbols <Note id={3}/>, but I've always found it hard to understand what it actually means. Let's try to understand it intuitively within our setup.
...
Concretely, we have a long vector $p(z)$ with a probability value for each possible $\mathbf{z}$ in the latent space. With each observation, we update _every_ value in $p(z)$.
...
Conversely, if $\mathbf{z}$ leads me to think that $\mathbf{x}$ is unlikely, say 20%, while the averaged opinion is 60%, then $\mathbf{z}$ did worse than the average. Let's decrease its assigned probability. $p(\mathbf{x} | \mathbf{z}) = 0.2$ and $p(\mathbf{x}) = 0.6$:
...
The two most popular methods for approximating complex distributions are [Markov Chain Monte Carlo (MCMC)](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo) and **variational inference**. We talked about MCMC previously [in](/protein-evolution/#generating-new-sequences) [various](/protein-representation/#using-the-representation) [contexts](/protein-hallucination). It uses a trial-and-error approach to generate samples, from which we can approximate the underlying complex distribution.

in contrast, variational inference looks at a collection of distributions. For example, we assume the observations follow a normal distribution and consider all distributions we get by varying the the mean and variance.

<Figure content={<VariationalInference />}></Figure>

Variational inference is a principled way to _vary_ these parameters of the distribution (hence the name!) and find a setting of them that best explains the observations.
...
If you're not interested in the mathematical details, this section can be safely [skipped](http://localhost:8000/variational-autoencoder/#what-does-elbo-mean) entirely. TLDR: expanding out $D_{KL}(q(z | \mathbf{x}) || p(z | \mathbf{x}))$ yields the equation at the end of the section, the foundational equation of variational inference.
...
Let's think about the left hand side of the equation. Maximizing ELBO has 2 effects:

1. increase $\log p(\mathbf{x})$. This is our basic intuition: since we just observed $\mathbf{x}$, $p(\mathbf{x})$ should go up!

2. minimize $D_{KL}(q(z | \mathbf{x}) || p(z | \mathbf{x}))$, which satisfies our goal of approximating the posterior.

### Connections with encoder-decoder

We have now derived VAEs from two angles: machine learning (encoder-decoder, reconstruction loss, regularization) and variational inference (Bayesian updates, evidence maximization, posterior approximation). They are two sides of the same coin. Let's explore the connections:

- In variational inference, we used the distribution $q(z | \mathbf{x})$ to approximate the posterior $p(z | \mathbf{x})$. This _is_ the encoder. The flexibility of a neural network encoder lets us approximate the complex posterior well.

- $p(x | \mathbf{z})$ is the decoder. The average (log-)probability of perfect reconstruction under our encoder-decoder model, $\mathbb E_{\mathbf{z} \sim q(z|\mathbf{x})} \left[\log p(\mathbf{x} | \mathbf{z})\right]$, falls out, perhaps unsurprisingly, as a good proxy for the intractable evidence, $\log p(\mathbf{x})$, that we want to maximize.

- In variational inference, p(z) is the our prior. Before any observation, believing $p(z) \sim Normal(0, 1)$ seems reasonable. It's a starting point.
