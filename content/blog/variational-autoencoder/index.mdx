---
title: Latent variable models for proteins
date: "2023-10-14"
description: Prediction protein function using deep generative models. Latent variable models, Variational Autoencoders (VAEs), Evidence Lower Bound (ELBO), mutation effect prediction.
---
...
Given an unknown input sequence $\mathbf{x}$, $p(\mathbf{x})$ describes how likely it is for $\mathbf{x}$ to be a functional protein given the evolutionary sequences in our MSA – it's a useful predictor of fitness. (If this sounds unfamiliar, check out <Link to="/protein-evolution">this post</Link> that applies pairwise models in the same way.)
