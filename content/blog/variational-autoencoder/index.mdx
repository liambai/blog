...
>
  The encoder-decoder model. The model tries to reconstruct the input via its
  internal representation. In this case, it incorrectly reconstructs "Dog" as
  "Dah".
</Figure>
...
A note on notation: when talking about probability, I find it helpful to make explicit whether something is fixed or a variable in a distribution. I will make fixed things **bold**. For example, $\mathbf{z} = [0.12, -0.25, -0.05, 0.33, 0.02]$ is a fixed vector, $p(x|\mathbf{z})$ is a conditional distribution over possible values of $x$. $p(\mathbf{x})$ is a number between $0$ and $1$ (a probability) while $p(x)$ is a distribution, i.e. a function of $x$.
...
>
  The encoder and decoder are both conditional distributions.
</Figure>

### Evaluation reconstruction

A good model at reconstruction gets it exactly right with high probability. Given some input $\mathbf{x}$, let's pick some random $\mathbf{z_{rand}}$ and look at $p(\mathbf{x}|\mathbf{z_{rand}})$: the probability of reconstructing the input perfectly. We want this number to be big.
...
In machine learning, we're used to _minimizing_, so let's define a loss function whose minimization is equivalent to maximizing ELBO:
...
>
  ...
  VAEs learn functions like $g$ that create arbitrary complex distributions.
  Figure from https://arxiv.org/abs/1606.05908.
</Figure>
...
Variational autoencoders are actually deeply rooted in a field of statistics called [**variational inference**](https://en.wikipedia.org/wiki/Variational_Bayesian_methods), which provides the first principles behind these decisions. That is the subject of the next section. However, since we already have a working machine learning model, feel free to [skip it](#modeling-protein-sequences) if you are more interested in how VAEs are applied.
...
$p(z)$ is the **prior distribution** (before the update) and $p(z | \mathbf{x})$ is the **posterior distribution** (after the update). With each observation, the baby computes the posterior and uses it as the prior for the next observation. This is called **Bayesian inference** because to compute the posterior, we use **Bayes rule**:
...
(When the latent space is massive, as in our case, $\mathbf{x}$ is infeasible to compute.)
...
In contrast, variational inference looks at a family of distributions to tries to pick the best one. For illustration, we assume the observations follow a normal distribution and consider all distributions we get by varying the the mean and variance.
