...
The two most popular methods for doing this approximation are Markov Chain Monte Carlo (MCMC) and variational inference. We talked about MCMC previously in <Link to="/protein-hallucination">various</Link> <Link to="/protein-evolution">contexts</Link>. It uses a trial-and-error approach to generate samples from the complex distribution, from which we can approximate the underlying distribution.
...
Given some fixed $\log p(\mathbf{x})$, our goal of minimizing $D_{KL}(q(z | \mathbf{x}) || p(z | \mathbf{x}))$ is equivalent to maximizing $ELBO(\mathbf{x})$.
