...
We can think of these bars (probabilities) as knobs we can tweak to adjust our model to better fit each new observation (without losing sight of previous ones).
...
Either by promoting an advocate of $\mathbf{x}$ or demoting a naysayer, we 1) adjust the latent distribution $p(z)$ to be more amenable to $\mathbf{x}$ and 2) bring up the average opinion, $p(\mathbf{x})$.
...
In contrast, variational inference looks at a family of distributions. For illustration, we assume the observations follow a normal distribution and consider all distributions we get by varying the the mean and variance.

<Figure content={<VariationalInference />}></Figure>

Variational inference is a principled way to _vary_ these parameters of the distribution (hence the name!) and find a setting of them that best explains the observations. In practice, the distributions are much more complex.
...
If you're not interested in the mathematical details, this section can be [skipped](#what-does-elbo-mean) entirely. TLDR: expanding out $D_{KL}(q(z | \mathbf{x}) || p(z | \mathbf{x}))$ yields the foundational equation of variational inference at the end of the section.
...
We have now derived VAEs from two angles: machine learning (encoder-decoder, reconstruction loss, regularization) and variational inference (Bayesian updates, evidence maximization, posterior approximation). They are two sides of the same coin. Let's explore some connections:
...
>
  ...
  <tbody>
    ...
    <tr>
      ...
      >
        $p(x | \mathbf{z})$ is the decoder, also a neural network. $\mathbb E_{\mathbf{z} \sim q(z|\mathbf{x})} \left[\log p(\mathbf{x} | \mathbf{z})\right]$ is the probability of perfect reconstruction. Striving for perfect reconstruction not only makes intuitive sense, but is also grounded in the variational reasoning on the left.
      </td>
    </tr>
    <tr>
      <td
      ...
      >
      $p(z)$ is the prior we use before seeing any observations. $p(z) \sim Normal(0, 1)$ is a reasonable choice. It's a starting point. It would take a lot of observations that disobey $Normal(0, 1)$ to, via Bayesian updates, convince us of a drastically different distribution.
      </td>
      <td
        style={{
          border: "2px solid",
      ...
      To prevent this, we constantly nudge the encoder $q(z | \mathbf{x})$ towards $Normal(0, 1)$, as a reminder of *where it would have started* if we were using Bayesian inference. When viewed this way, $D_{KL}(q(z | \mathbf{x}) || p(z))$ is a *regularization term*.
      </td>
    </tr>
  </tbody>
</table>
