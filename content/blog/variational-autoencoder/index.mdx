...
The latent space has fewer dimensions than the inputs, so encoding can be viewed as a form of [data compression](https://en.wikipedia.org/wiki/Data_compression). The baby doesn't retain all the details of each syllable heard – the intricate patterns of each sound wave – only their salient features, a compression.
...
<Figure content={<VariationalInference />}>
  ...
  observations (blue dots). In essense, varational inference is all about doing
  these adjustments.
</Figure>
...
Also, check out the cool [paper](https://www.nature.com/articles/s41592-018-0138-4) from the Marks lab applying VAEs to protein sequences. You should have the theorical tools to understand it well.
