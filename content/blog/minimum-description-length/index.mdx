...
Here's one way to define complexity, called **Kolmogorov complexity**: the length of a string is the _shortest computer program_ in some predefined language that outputs it. The choice of programming language turns out to matter very little, so let's illustrate with Python:
...
3. To get $01110100110100100110$, we have to type out the whole string as-is:
...
Making this mathematically rigorous takes some work – we need to define the language, measure the length of programs in bits, etc. – and in all but the simplest cases, finding the optimal program and spelling it out is intractable. But that's the basic idea: an object is complex if we need a long Python function to return it. This function is also called a **description** of the string. Assuming we've got the shortest, its length is the string's **minimum description length**.

What separates these strings from each other? For 1 and 2, we can exploit their regularity (the repeating pattern) to represent them in a more compact way; for 3, we can't. Therefore, Kolmogorov complexity can be understood in terms of **data compression**.

- A string is complex if it is hard to compress.
- The most optimal compression algorithm gives us a minimum description whose length is the Kolmogorov complexity.

Here is a bold claim that I'll back up throughout the rest of this post: compression and _learning_ are the same thing. We have learned the simple essence of the first string by writing it as `"0" * 20`. Having to spell out the third string verbatim, we've learned nothing.
...
As a stats major, my knee-jerk reaction to these lines is: left & right bad, middle good! Under/over-fitting, unseen data, bias/variance tradeoff... you name it. But beyond these arguments, there's another way to see why the middle one is best, though the lens of complexity: it offers the _shortest description_ of these points.
...
The descriptions above are quite hand-wavy. How can we quantify the intuition that the errors of the 1st degree polynomial are "harder to explain" than the errors of the 3rd degree polynomial? The next section uses tools from information theory to make this calculation precise.
...
One idea from information theory is to use a shorter code for the more likely outcomes like `0` and `0.1`, and a longer code for the those rare events like `1.7976931348623157e+308`. [Shannon](https://en.wikipedia.org/wiki/Claude_Shannon) even proved that the optimal code length is $-\log_2(p(x))$, where $p(x)$ is the probability of event $x$.
...
For example, if you think a number comes up as often as 50% of the time, you should represent it with only 1 bit.
...
Back to the claim on compression = learning: perhaps you can agree that these two summaries of this example are two sides of the same coin:

1. We have _compressed_ these points using a 3rd degree polynomial, which lets us describe them in very few bits.
2. We have _learned_ a good function approximation of these points, a 3rd degree polynomial, which models the underlying distribution.
