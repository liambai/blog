...
<Figure content={<Image path={require("./images/compression.png")} />}>
  ...
  considers the fact that LLMs have a limited context window whereas gzip can
  operate on a much larger range and exploit more compressible patterns.
</Figure>
...
More remarkably, even though Llama and Chinchilla are not trained primarily trained on text, they are quite good at compressing image patches and audio samples, outperforming specialized algorithms such as [PNG](https://compress-or-die.com/Understanding-PNG).
...
The compression efficiency of LLMs come at a cost: the size of their weights. This is shown on the right half of the table: "Adjusted Compression Rate". Technically, the minimum description length includes these weights in its $L(H)$ term, like we coded the polynomial coefficients in addition to the errors. Practically, we don't want to lug around all the Llama weights (up to 100s of GBs) every time we compress a file. <Note id={1}/>
