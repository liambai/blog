...
<Figure content={<LongRangeContacts />}>
  ...
  the fact that residues 2 and 3 (close to the diagonal of the matrix) are close
  in space is obvious and uninteresting, but the fact that 2 and 8 (far from the
  diagonal of the matrix) are close in space – due to some interresidue
  interaction represented by the blue link – is important for structure.
</Figure>
