...
**transform-restrained Rosetta (trRosetta)** is a structure prediction model that, like almost everything we'll talk about in this post, was developed at the [Baker lab](https://www.bakerlab.org/). trRosetta has 2 steps:
...
Here's the architecture of the trRosetta CNN. For our purposes, understanding the inner workings is not as important; we can just keep in mind the big picture: the network takes in an MSA and spits out these interresidue distances and orientation angles.
...
Without a canonical example of sharpness, we can quantify it by coming up with a canonical blurry distance map $Q$ – a bad example – to steer away from: a distance map is sharp if it's very _different_ from $Q$ <Reference id={2}/>.

We can get $Q$ from a **background network**, which is the same as trRosetta with one important catch: the identity of each amino acid is hidden in the training data. The background network retains some rudimentary information about the amino acid chain, e.g. residues that are close in sequence are close in space. But it cannot learn anything about the interactions between amino acids that come from their unique chemistries.
...
This is the **Kullback–Leibler (KL) divergence**, which came from [information theory](https://en.wikipedia.org/wiki/Information_theory). It's a common [loss function](https://pytorch.org/docs/stable/generated/torch.nn.KLDivLoss.html) in machine learning. (Note on notation: it's conventional to use double bars like $D_{KL}(P || Q)$ to emphasize the fact that $D_{KL}(P || Q) \neq D_{KL}(Q || P)$, i.e. KL divergence is not a true distance metric.)
...
(Notation is slightly rewritten to be consistent with <Reference id={2}/>) Intuitively, $P$ is sharp if it's as far away from $Q$ as possible, as measured by the average KL divergence.
...
>
  For one of the hallucinated sequences, we compare trRosetta's predicted
...
There's just the thing in deep neural networks like trRosetta: **gradients** <Reference id={3}/>. During training, gradients guide trRosetta in adjusting its parameters to make better structure predictions <Note id={3}/>.
...
Most proteins perform their function via a **functional site** formed by a small subset of residues called a **motif**. For example, the functional sites of enzymes bind to their substrates and perform the catalytic function <Note id={4}/>.
...
SARS-CoV-2, the virus behind the Covid-19 pandemic, has a clever way of entering our cells. It takes advantage of an innocent, blood-pressure regulating protein in our body called [angiotensin-converting enzyme 2 (ACE2)](https://en.wikipedia.org/wiki/Angiotensin-converting_enzyme_2) that straddles the cell membrane.
