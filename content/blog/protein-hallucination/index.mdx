...
Repeat this thousands of times: we'll end up with an image that looks a lot like bananas! This is the essence of MCMC, which lets us turn our monkey into a **generative model**.
...
Let's replace the banana recognition task with one we're not so good at: predicting the activity of proteins – and creating new ones with desired properties. The ability to do this is revolutionary to industrial biotechnology and therapeutics. In this post, we'll explore how approaches similar to DeepDream can be used to design new proteins.
...
If we know these 6 numbers for each residue pair in the amino acid chain, then we should have a pretty good sense of the 3D structure, providing a good foundation for step 2.
