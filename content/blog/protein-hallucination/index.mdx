...
To get $Q$, we can train trRosetta on the same data, except with one catch: we hide the identity of each amino acid. This retains some rudimentary characteristics of an amino acid chain: each amino acid has distance $0$ to itself (the dark diagonals), residues that are close in sequence are close in space, etc. But it eliminates all information about the interactions between amino acids enabled by their unique chemistries.
...
Folks at the Baker lab then expressed these hallucinated sequences in _E. coli_ and found that many of them folded into stable structures that closely match trRosetta's predictions.
...
All we need is a loss function: our average KL divergence between $P$ and $Q$ is a natural one. At each step:
...
Specifically, it binds to an [alpha helix](https://en.wikipedia.org/wiki/Alpha_helix) in ACE2 that is exposed to the outside. And then enters the cell like this:
...
This is exactly our functional site scaffolding problem. Folks at the Baker lab used the composite loss function approach and hallucinated these receptor traps on the right.

<Figure
  content={<Image path={require("./images/ACE2-designs.png")} width="80%" />}
></Figure>

I hope I have convinced you that these hallucinations are not only cool but also profoundly impactful. It was only in the past few years that we have acquired the ability to engineer proteins with this level of control, thanks primarily to the groundbreaking work from the Baker lab.

And of course, this is only the tip of the iceberg: the ability to engineer these binders to disrupt disease mechanisms will revolutionize drug discovery and reduce a lot of suffering in the world.
