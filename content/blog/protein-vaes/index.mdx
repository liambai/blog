...
The evolutionary history of a protein – in the form of an [Multiple Sequence Alignments (MSAs)](https://en.wikipedia.org/wiki/Multiple_sequence_alignment) - holds clues to its structure and function. An example: positions that co-vary in the MSA tend to interact with each other in the folded structure, often via direct 3D contact.

<Figure content={<MSACoupling />}>
  An MSA contains many different variants of a protein sequence sampled by
  evolution. The structure sketches how the amino acid chain might fold in space
  (try dragging the nodes). Hover over each row in the MSA to see the
  corresponding amino acid in the folded structure. Hover over the blue link to
  highlight the contacting positions.
</Figure>

In a previous <Link to="/protein-evolution">post</Link>, we talked about ways of capturing information like this hidden in MSAs using [pair-wise models](https://en.wikipedia.org/wiki/Potts_model) that account for every possible pair of positions. But what about the interactions between 3 positions? Or even more? Those higher-order interactions are commonplace in natural proteins but account for them all is unfortunately computationally infeasible.

This post is about a solution to this problem of tracking these far-too-numerous higher-order terms – based on a machine learning tool called **variational autoencoders (VAEs)**. If you're new to VAEs, check out this <Link to="/variational-autoencoder">deep dive</Link>.
...
1. No BS. You have to actually capture something meaningful about the input sequence. As a test, a neural network (called a **decoder**) needs to be able to decode from $\mathbf{z}$ what the input sequence was, reasonably well. TODO: note on joint training. This is called **reconstruction**.
