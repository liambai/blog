...
Through billions of years, evolution introduces random mutations to every protein. All else can change, but the binding pockets almost never do. This is perhaps unsurprising: they are the parts that actually do the work! Spoons come in different shapes and sizes, but the part the scoops never changes.
...
One possible explanation: these correlated positions form a binding pocket important to the protein's function. A willy-nilly mutation to one position disrupts the binding pocket, which renders the protein useless. Poor organisms that carried that mutation are dead and therefore absent from the MSA.

In a previous <Link to="/protein-evolution">post</Link>, we talked about ways of learning information like this from MSAs using [pair-wise models](https://en.wikipedia.org/wiki/Potts_model) that account for every possible pair of positions. But what about the interactions between 3 positions? Or even more? Binding pockets, after all, are made up of many positions. Unfortunately, accounting for all these possible combinations in this way is computationally impossible.

This post is about a solution to this problem of tracking these far-too-numerous combinations – using a tool from machine learning called **variational autoencoders (VAEs)**. If you're new to VAEs, check out this deep dive.
...
Imagine some vector $\mathbf{z}$, a **latent variable**, that distills all the information in the MSA: pairwise, any 3 positions, anything.
...
To get this magical $\mathbf{z}$, let's ask a neural network called an **encoder**. We don't know how it's done – nor do we really care – as long as it's done! That's the key idea behind VAEs.

### VAEs in a nutshell

Given a protein sequence, let's ask the encoder: Can you capture (in $\mathbf{z}$) the salient features of this protein? Biologically, this might mean things like: which residues form a binding pocket. There are 2 rules:
...
2. No rote memorization. If you merely memorize the input sequence, you'll be great at the first rule but you won't be any help for sequences you've never seen before. This is called **regularization**.
...
What do we actually mean by _model_? Abstractly, our model captures information about a specific protein, via its evolutionary history (MSA). Concretely, our model is a function that takes a protein sequence $\mathbf{x}$ and spits out a probability, $p(\mathbf{x})$:

$$
\mathbf{x} \rightarrow p(\mathbf{x})
$$

With training, we want this probability to approximate how likely it is for the input sequence to have come from our MSA. In other words, how likely is it to be a functional variant of our protein?

This probability is the collaborative work of our encoder and decoder, which are trained together.

$$
\mathbf{x} \xrightarrow{encoder} \mathbf{z} \xrightarrow{decoder} p(\mathbf{x})
$$

An accurate model enables us to make predictions about disease variants we've never seen before, or even engineer new ones with desired properties. More on this later!

### Training & inference

During training, we show the MSA to our VAE one sequence at a time. For each sequence, we compute the loss function and use gradient descent to update the encoder and decoder parameters. Repeating this many times gives our VAE a good idea of what it's like to be this protein.

When given an unknown input sequence, we pass it through the VAE in the same way and produce a probability for the input sequence (green arrow).
...
Given an unknown input sequence $\mathbf{x}$, $p(\mathbf{x})$ is a measure of its fitness. $p(\mathbf{x})$ is low, $\mathbf{x}$ probably won't even fold into a working protein. If $p(\mathbf{x})$ is high, $\mathbf{x}$ is probably
