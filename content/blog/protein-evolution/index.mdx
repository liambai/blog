...
date: "2023-09-16"
description: Predicting protein structure and function. Multiple Sequence Alignments (MSAs), the Potts model, EVCouplings.
---
...
import PottsImage from "./PottsImage.jsx"
...
$E$ is often called an **energy function** – it's not a probability distribution, but we
...
To disentangle such direct and indirect correlations, we want to have a more _global_
model that accounts for _all_ pair correlations. [EVCouplings](https://evcouplings.org/)
is a protein structure and function prediction tool that uses an approach called
**mean-field approximation** (from [physics](https://en.wikipedia.org/wiki/Mean-field_theory);
[paper](https://www.pnas.org/doi/10.1073/pnas.1111471108) applying it to MSAs). The calculations
are straightforward:
...
a direct measure of interaction given $i$ and $j$, without a dependence on specific amino acids. In
other words, we want to average over all possible pairs of amino acids that can inhabit the two
positions $i$ and $j$. To do this in a principled and effective way, we use a concept called
**mutual information**:
...
As we talked about above, the local nature of $f_{ij}$ can be limiting: for one, it's bad at
discerning transitive correlations. [EVCouplings](https://evcouplings.org/), for example,
uses a different quantity to approximate the probability that $i$ and $j$ are in contact:
...
where the $J_{ij}$'s are the global interaction terms we obtained by mean-field approximation, and the
$\tilde{h}$ terms can be calculated by requiring the following constraints
...
1. Let's fix the amino acid at position $i$ to be L. Consider $P_{i j}^{D i r}(L, \mathrm{A_j})$
   for all possible $A_j$'s if we sum them all up, we should get the probability of observing $L$
   independently at position $i$, i.e. $f_i(L)$.

2. The same idea but summing over all $A_i$'s.

Once we have $P_{i j}^{D i r}$, we can average over all possible $A_i$'s and $A_j$'s in the same way
as we did for mutual information:
...
In searching for a good model of the protein family, we came up with a way to measure
how much two positions interact with each other: $J_{ij}$. Can we apply it to the protein folding
problem?

Our $J_{ij}$ is a function that takes in two amino acids: $J_{ij}(A_i, A_j)$; however, we would like
a direct measure of interaction given $i$ and $j$, without this dependent on specific amino acids.
