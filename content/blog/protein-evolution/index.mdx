...
<Image
  path={require("./sequence-evolution.png")}
  caption="
    Diagram from Roshan Rao's awesome [dissertation talk](https://youtu.be/hcJS9d09ECA?si=DXLsnOvbJH7wwrJ1)
    on these topics.
  "
/>
...
<MSACoupling
  caption="
    The structure sketches how the amino acid chain might fold in space (try dragging the nodes). 
    Hover over each row in the MSA to see the corresponding amino acid in the folded structure.
    Hover over the blue link to highlight the contacting positions. 
  "
/>
...
<Distributions
  caption="
    ...
    higher probabilities. The set of all possible sequences (the 
...
If we compile these frequencies into a matrix, we get what is known as a
[**position-specific scoring matrix (PSSM)**](https://en.wikipedia.org/wiki/Position_weight_matrix),
commonly visualized as a [sequence logo](https://en.wikipedia.org/wiki/Sequence_logo).

<Image
  path={require("./sitewise-logo.png")}
  caption="
    A sequence logo [generated](https://weblogo.berkeley.edu/logo.cgi) from our MSA.
    The height of each amino acid is an indicator of its evolutionary conservation.
  "
  width="90%"
/>

To evaluate some new sequence $A$ of $L$ amino acids, we can look at
...
<MSACovariance
  caption="
    Hover over the MSA to compute pairwise amino acid frequencies in reference to the second column.
  "
/>
...
<Image
  ...
  caption="
    ...
    Figure from [https://arxiv.org/abs/1511.03031](https://arxiv.org/abs/1511.03031).
  "
  width={300}
/>
...
so important that failure for a sequence to do so is a death sentence that explains its absence in
...
come with their own tradeoffs). I will dive into them in a future post.
...
of attention called **axial attention** [6] works really well on MSAs [7][8], giving rise to
models with even better performance. I also hope to do a deep dive on this soon!
