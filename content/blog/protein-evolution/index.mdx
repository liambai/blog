...
at each position. Let $f_i(A_i)$ be the frequency of observing the amino acid $A_i$
at position $i$.
...
Actually, physicists have studied this energy function since the 1950s, in a different
...
magnitude is big if they interact, small if they don't; the $h_i$ terms capture the energy
...
and they are in contact in the folded protein. Intuitively, it's important to maintaining the
function of the protein that when one position changes, the other also changes in a specific way;
failure to do so is likely a death sentence explaining the absence in our MSA. Let's use the tools
we have to quantify this co-variance.
...
I hope by now you can join me in the fascination and appreciation of MSAs.
...
better performance. I hope to do a deep dive on this soon!
