...
Here's an example of signals hidden in MSAs: amino acid positions that tend to co-vary in the MSA tend to interact with each other in the folded structure, often via direct 3D contact. We'll make this idea concrete.
...
<Distributions
  caption="
    Hover over the bars to see the probabilities. Sequence probabilities are made up but follow some expected patterns: sequences that resemble sequences in the MSA have higher probabilities. The sequence space is mind-bendingly vast: the number of possible 10 amino acid sequences is 20^10 (~10 trillion) because there are 20 amino acids; the bar graph is very truncated.
  "
/>
...
<Image
  path={require("./images/sequence-logo.png")}
  caption="
    A sequence logo [generated](https://weblogo.berkeley.edu/logo.cgi) from our MSA. The height of each amino acid is an indicator of its degree of evolutionary conservation.
  "
  width="90%"
/>

To evaluate some new sequence $A$ of $L$ amino acids, let's look at:
...
Earlier, we considered using $f_{ij}$ as the term capturing pairwise interactions. $f_{ij}$ focuses on what's happening at positions $i$ and $j$ – nothing more. It's a _local_ measurement. Imagine this: positions $i$ and $j$ each independently interact with position $k$, though they do not directly interact with each other. With this **transitive correlation** between $i$ and $j$, the nearsighted $f_{ij}$ would likely overestimate the interaction between them.
...
The theory behind these steps is involved and beyond our scope, but intuitively, we can think of the matrix inversion as an averaging step that incorporates all pair correlations globally. This method is called [**Direct Coupling Analysis (DCA)**](https://en.wikipedia.org/wiki/Direct_coupling_analysis).
...
<Image
  path={require("./images/DI-vs-MI.png")}
  caption="
...
<Image
  path={require("./images/structures.png")}
  caption="
...
<Image
  path={require("./images/deltaE-mutations.png")}
  caption="
...
<Image
  path={require("./images/deltaE-experimental.png")}
  caption="
...
<Image
  path={require("./images/CM-energy.jpeg")}
  caption="
