...
<Figure content={<Distributions />}>
  ...
  MSA have higher probabilities. The sequence space is mind-bendingly vast: the
  number of possible 10 amino acid sequences is 20^10 (~10 trillion) because
  there are 20 amino acids; the bar graph is very truncated.
</Figure>
...
<Figure content={<MSACoupling />}>
  ...
  dragging the nodes). Hover over each row in the MSA to see the corresponding
  amino acid in the folded structure. Hover over the blue link to highlight the
