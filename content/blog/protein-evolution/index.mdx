...
breakthroughs like [AlphaFold](https://www.forbes.com/sites/robtoews/2021/10/03/alphafold-is-the-most-important-achievement-in-ai-ever/?sh=6e0571586e0a)
...
position $k$, though they do not directly interact with each other. Because of this
...
failure to do so is likely a death sentence that explains the absence in our MSA. Let's use the tools
...
sequences with low energy should be more likely to fail. Let $A^{\mathrm{wt}}$ be a wildtype,
...
mutant, we can calculate some $\Delta E$'s first before stepping in the lab and cam perhaps
focus only on the sequences with more positive $\Delta E$'s that are more likely to work.
...
$P(A)=\frac{1}{Z} \exp(E(A)/T)$ (slightly modified to include $T$). Why this works involves a lot of
cool math that we won't have time to dive into now. This is the
