...
with, let's try to directly sample from our probability distribution like this:
...
3. Compare $E(A)$ with $E(A^{\mathrm{cand}})$.
   ...
   - if energy decreased, still accept the candidate with some probability proportional the energy
     ...
     - $T \in (0, 1]$ lets us control how forgiving we want to be: $T = 1$ makes accepting more likely;
       $T \to 0$ makes accepting less likely.
...
cool math beyond our scope. This is the
...
samples from the DCA distribution and tested them experimentally.
